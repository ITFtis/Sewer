...
namespace Map.Other
{
    public class proxy : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            //uri = @"https://ldgis.tainan.gov.tw/PortalAPI/WMS/A20190423-0001/00005-4780ccc1-3669-476b-b364-c449141a5374/Wub1FVt52fFnUfKD5ExF5TA0NjIyNDUw/";
            ...
            //*********使用kml cache***********
            ServerRootPath =context.Server.MapPath("~/");
            if (!kmlCache.ContainsKey(uri))
            {
                if (uri.IndexOf("http://utility.arcgis.com/sharing/kml?") >= 0)
                {
                    string fn = Path.Combine(ServerRootPath, @"Data\" + uri.Substring(uri.IndexOf("url=") + 4, uri.IndexOf("&") - uri.IndexOf("url=") - 4).Replace("kmz", "json"));
                    if (File.Exists(fn))
                    {
                        DateTime st = new DateTime();
                        Debug.WriteLine("讀取cache檔:" + fn );
                        using (StreamReader sr = new StreamReader(fn))
                        {
                            kmlCache.Add(uri, sr.ReadToEnd());
                            sr.Close();
                        }
                        Debug.WriteLine("\t共花:" + (DateTime.Now - st).TotalMilliseconds + "毫秒");
                    }
                }
            }
            if (kmlCache.ContainsKey(uri)) ///cache kml
            {
                
                response.ContentType = "ContentType:text/plain;charset=UTF-8";
                response.Write(kmlCache[uri]);
                Debug.WriteLine("使用cache:" + uri);
                response.End();
            ...
            if (passThrough || string.IsNullOrEmpty(token) || hasClientToken)
                ...
                fetchAndPassBackToClient(serverResponse, response, true, uri);
            else
            {
                ...
                bool tokenRequired = fetchAndPassBackToClient(serverResponse, response, false,uri);
        ...
        */
        ...
        private System.Net.WebResponse forwardToServer(HttpContext context, string uri, byte[] postBody)
        {
            return
                ...
                doHTTPRequest(uri, context.Request.HttpMethod, context.Request.ContentType); //新增第3個參數(取無參數的WS)
        }

        static System.Collections.Generic.Dictionary<string, string> kmlCache = new System.Collections.Generic.Dictionary<string, string>();
        static string ServerRootPath = null;
        private bool fetchAndPassBackToClient(System.Net.WebResponse serverResponse, HttpResponse clientResponse, bool ignoreAuthenticationErrors,string uri)
        {
            if (serverResponse != null)
            {
                ...
                using (Stream byteStream = serverResponse.GetResponseStream())
                {
                        ...
                        serverResponse.ContentType.Contains("xml"))
                    {
                        using (StreamReader sr = new StreamReader(byteStream))
                        {
                            ...
                            //*********使用kml cache***********
                            if (uri.StartsWith(@"http://utility.arcgis.com/sharing/kml?")) //ContentType:text/plain;charset=UTF-8
                            {
                                kmlCache.Add(uri, strResponse);
                                string fn = Path.Combine(ServerRootPath,@"Data\"+ uri.Substring(uri.IndexOf("url=") + 4, uri.IndexOf("&") - uri.IndexOf("url=") - 4).Replace("kmz", "json"));
                                try
                                {
                                    ...
                                    Debug.WriteLine("cache kmz OK:"+uri);
        ...
        private System.Net.WebResponse doHTTPRequest(string uri, string method, string contentType=null)
        {
            byte[] bytes = null;
            ...
            if (method.Equals("POST"))
            {
                ...
                {
                    _contentType = _contentType == null || "".Equals(_contentType) ? "application/x-www-form-urlencoded": _contentType;
            ...
            return doHTTPRequest(uri, bytes, method, PROXY_REFERER, _contentType);
        }
